...
static std::shared_ptr<IALSHelper> mOplusALSHelper;
void AlsCorrection::init() {
    ...
    if (!isSupportOplusSysHelper) {
        ALOGE("Oplus System Helper is NOT Declared!!");
        abort();
    }
...
void AlsCorrection::process(Event& event) {
    ...
    */
    ...
    if (conf.rgbw_max_lux[0] == 0.0 && conf.rgbw_max_lux[1] == 0.0 &&
        ...
        ALOGE("Oplus: Maybe EngSensor not finished when init sensorservice, Trying to init AlsCorrection again! This event will be dropped!");
		AlsCorrection::init();
